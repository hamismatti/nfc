...
class App extends React.Component {
  ...
  readData = async () => {
    try {
      ...
      let resp = await NfcManager.requestTechnology(tech, {
        alertMessage: 'Ready to read custom NXP!',
      });
      ...
      let payloadLength = Number(resp.toString().split(',')[1]);
      ...
      for (let i = 0; i < bytes.length; i++) {
        ...
        if (Number(bytes[i]) === 254) {
          break;
        }

        text = text + String.fromCharCode(Number(bytes[i]));
      }
  ...
  writeData = async () => {
    ...
    try {
      let tech = Platform.OS === 'ios' ? NfcTech.MifareIOS : NfcTech.NfcA;
      let resp = await NfcManager.requestTechnology(tech, {
        alertMessage: 'Ready to write custom NXP cmd!',
      });
